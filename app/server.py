...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if self.path=='/api/groups': call(board.set_group(body['name'],body['prefix'],body['pool'])); return self.send_json({'ok':True,'message':'Rental group saved; matching workers will inherit this pool.'})
            if self.path=='/api/groups/delete': call(board.delete_group(body['name'])); return self.send_json({'ok':True})
            self.send_error(404)
